...
TEST_PATH = 'astrohack/testsubmission copy.csv'
...
# submission = pd.read_csv('submission.csv', delimiter=',', names=['SDSS_ID','logMstar'], header=0)
# test = pd.read_csv('test.csv', delimiter=',', names=['SDSS_ID','logMstar', 'err_logMstar'], header=0)
...
def calculate_full_score(submission_path, test_path=TEST_PATH):
    ...
    test = pd.read_csv(test_path, delimiter=',', names=['SDSS_ID', 'logMstar', 'err_logMstar'], header=0)
    comparison = pd.merge(submission, test, on='SDSS_ID', how='right')
    comparison.fillna(0, inplace=True)

    # filter -99 values (not in submissions, else it's easy to get chi2=0 as a score)
    comparison = comparison[(comparison.logMstar_y != -99) & (comparison.err_logMstar != -99)]
